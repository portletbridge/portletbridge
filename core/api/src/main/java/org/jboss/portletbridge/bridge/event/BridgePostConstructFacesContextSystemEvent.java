...
/**
 * Faces <code>SystemEvent</code> sent by the Bridge in its <code>doFacesRequest()</code> method after the bridge has
 * acquired the <code>FacesContext</code> but before it runs the <code>Lifecyle</code>. Gives listeners the ability to
 * do work at a point in which the entire request processing environment has been established but before actually
 * executing the request. One use allows <code>ExternalContext</code> implementations to be notified prior to use to do
