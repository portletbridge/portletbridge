...
public interface Bridge {
  ...
  /**
   * Special token parameter in the url passed to the bridge's ExternalContext.encodeActionURL()
   * that it recognizes as an indication that this action should be treated as a direct link
...
/**
   ...
   * to render portlets in this application.  A policy of DEFAULT indicates
   * the bridge will first delegate rendering and if this results in an
   * exception being thrown will render the itself.  A policy of ALWAYS_DELEGATE
   * indicates the bridge will always delegate rendering, never rendering itself.
   * A policy of NEVER_DELEGATE indicates the bridge will always render itself without
   * delegating.
   */
   ...
   *              if an exception has occurred that interferes with the bridge's normal operation.
   *              For example, if the bridge is already initialized.
