...
public class PortletBodyRenderer extends Renderer {
	...
	private static final String[] BODY_PASS_THRU_ATTRIBUTES = new String[] {
			...
			"onmouseover", "onmouseup", "onunload", "styleClass", "title" };
	...
	/**
	 * It is forbidden for a portlet to render the &amp;&lt;body&amp;&gt;
	 * element, so instead, render a &amp;&lt;div&amp;&gt;element.
	 * 
	...
	public void encodeBegin(FacesContext facesContext, UIComponent uiComponent)
			throws IOException {
		logger.log(Level.FINER, "encodeBegin()");
		ResponseWriter responseWriter = facesContext.getResponseWriter();
		responseWriter.startElement(ELEMENT_DIV, uiComponent);
		String portletClientId = facesContext.getViewRoot()
		...
		for (int i = 0; i < BODY_PASS_THRU_ATTRIBUTES.length; i++) {
			...
			Object attributeValue = uiComponent.getAttributes().get(
					attributeName);
			...
			if (attributeName.equals(ATTR_STYLE_CLASS)) {
				...
				if (attributeValue == null) {
					...
					attributeValue = attributeValue.toString() + " "
							+ STYLE_CLASS_PORTLET_BODY;
				}
			}

			if (attributeValue != null) {
				responseWriter.writeAttribute(renderedName, attributeValue,
						attributeName);
			}
		}
	}
	...
	public void encodeEnd(FacesContext facesContext, UIComponent uiComponent)
		...
		List<UIComponent> uiComponentResources = uiViewRoot
				.getComponentResources(facesContext, TARGET_BODY);
