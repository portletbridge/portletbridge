...
public class PortletHeadRenderer extends Renderer {
    ...
    public void encodeBegin(FacesContext facesContext, UIComponent uiComponent) throws IOException {
        ...
        PortletBridgeContext portletBridgeContext = PortletBridgeContext.getCurrentInstance(facesContext);
        String portletName = portletBridgeContext.getBridgeConfig().getPortletName();
            ...
            for (UIComponent uiComponentResource : uiComponentResources) {
                ...
                if (resourceName != null ) {
                    ...
                    if (!isResourceAlreadyRendered(facesContext, resourceKey)) {
                        ...
                        if (CONTENT_TYPE_SCRIPT.equals(contentType)) {
                            ...
                            if (PortletContainerUtil.isMarkupHeadElementSupported(portletRequest)) {
                                ...
                                PortletContainerUtil.addStyleSheetResourceToHead(portletRequest, resourceURL, uid);
                            }
                        } else {
                    ...
                    if (RENDERER_TYPE_SCRIPT.equals(rendererType)) {
                        ...
                        // If the portlet container supports the Portlet 2.0 standard mechanism for adding elements to the
                        // <head>...</head> section of the rendered page, then get the script text from the resource and add it via the standard mechanism.
                        if (PortletContainerUtil.isMarkupHeadElementSupported(portletRequest)) {
                            PortletResponse portletResponse = (PortletResponse) externalContext.getResponse();
                            String resourceValueAsString = getResourceValueAsString(uiComponentResource);
                            PortletContainerUtil.addScriptTextViaMarkupHeadElement(portletRequest, portletResponse, resourceValueAsString);
                        } // Otherwise, we have no choice but to relocate the resource to the body, since there is no way to
     ...
     * @param  resourceLibraryName  The resource library name.
     * @return  An instance of a new resource associated with the specified name and library.
     */
    protected Resource createResource(FacesContext facesContext, String resourceName, String resourceLibraryName) {
        return facesContext.getApplication().getResourceHandler().createResource(resourceName, resourceLibraryName);
    }

    /**
     * Returns a URL for the specified resource that can be used to download the resource from the browser. See section
     * 13.1.1.2 of the JSF 2.0 Spec for guidance on rendering resource URLs.
     *
     * @param  resource  The JSF resource.
...
//        	resourceURL = externalContext.encodeResourceURL(resourceURL);
    ...
    /**
     * Gets the resource "library" name associated with the specified component resource.
     *
     * @param  uiComponent  The component resource.
    ...
    protected String getResourceLibraryName(UIComponent resourceUIComponent) {
        return (String) resourceUIComponent.getAttributes().get(COMPONENT_ATTR_LIBRARY);
    }

    /**
     * Gets the resource "name" associated with the specified component resource. Note that if the specified component
     * has a query-string, then the query-string will not appear in the return value. This is because script resources
     * may specify query-strings for internal purposes, but those query-strings have no meaning for rendered URLs.
     *
     * @param  uiComponent  The component resource.
    ...
    protected String getResourceName(UIComponent resourceUIComponent) {
        String resourceName = (String) resourceUIComponent.getAttributes().get(COMPONENT_ATTR_NAME);
    ...
    /**
     * Checks a request attribute in order to determine whether or not the resource associated with the specified key
     * has already been rendered (added) for inclusion in the <head>...</head> section of the portal page.
     *
     * @param  facesContext  The current faces context.
     * @param  resourceKey  The key associated with the resource.
     * @return  True if the resource has been rendered, otherwise false.
     */
    protected boolean isResourceAlreadyRendered(FacesContext facesContext, String resourceKey) {
        return (facesContext.getAttributes().get(resourceKey) != null);
    }

    /**
     * Sets a request attribute indicating that the resoruce associated with the specified key has already been rendered
